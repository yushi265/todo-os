...
REQUIREMENTS.md 6章・17章・18章に基づき、TODO一覧を手動で並び替えられるようにする。新規`PATCH /api/todos/reorder` APIと、`sortBy=manual`時のみ有効なカード・キーボード・タッチ操作を実装する。
...
- **差分**: `TodoListItem`/`CompletedTodoListItem`にカード全体のドラッグ・タッチ・キーボード操作を追加し、既存の6点リーダーも互換入口として残す。`TodoList`に各操作の状態管理と読み上げ通知を追加。
...
- **カード全体をドラッグ入口にする判断**: 6点リーダーを残しつつ、カードの非インタラクティブ領域も同じイベントへ接続する。ボタン・リンク・入力のクリックやフォーカス操作を奪わない。
