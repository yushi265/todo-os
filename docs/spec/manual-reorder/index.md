# manual-reorder: TODOの手動並び替え（カード操作・ドラッグ&ドロップ）

> 薄い実装 spec の入口。読み手は人間（Gate 1/2/3 承認者）。AI 実装エージェント（Codex）は [service.md](./service.md) / [ui.md](./ui.md) を読む。

## 概要

REQUIREMENTS.md 6章・17章・18章に基づき、TODO一覧を手動で並び替えられるようにする。新規`PATCH /api/todos/reorder` APIと、`sortBy=manual`時のみ有効な未完了カード・キーボード・タッチ操作を実装する。完了（`DONE`）・中止（`CANCELED`）は一覧の末尾に固定し、並び替え対象から除外する。
...
- [ ] **AC-4**: TODO一覧で、`sortBy=manual`の時のみ、表示中の未完了TODOカード（`TODO`/`IN_PROGRESS`）をカード全体からドラッグ&ドロップで並び替えられる。`DONE`/`CANCELED`は常に一覧の末尾に表示し、並び替え対象に含めない。ドロップ確定後、並び替え結果を反映した`todoIds`で`PATCH /api/todos/reorder`を呼び出す。
- [ ] **AC-5**: `sortBy`が`manual`以外の場合、カードのドラッグ操作は行えない。6点リーダー（ドラッグハンドル）はステータスやソート条件にかかわらず表示しない。
...
- [ ] **AC-8**: 手動並び替えが有効なカードをフォーカスし、Spaceで選択、上下矢印で移動、Spaceで確定、Escapeでキャンセルできる。操作状態は`aria-live`で読み上げ可能である。
- [ ] **AC-9**: タッチ端末ではカードの非インタラクティブ領域を長押しして並び替えを開始でき、カード内の通常のボタン操作は長押し判定を開始しない。
...
- **差分**: `TodoListItem`に未完了カード全体のドラッグ・タッチ・キーボード操作を追加する。`CompletedTodoListItem`はドラッグ対象外とし、`TodoList`で未完了を先、完了・中止を末尾に並べる。6点リーダーは表示しない。
...
- マウス操作はHTML5 Drag and Drop API（`draggable`/`onDragStart`/`onDragOver`/`onDrop`/`onDragEnd`）、タッチ操作は既存の長押し判定、キーボード操作はカードのフォーカスと矢印キーを使う。新規ライブラリは追加しない（YAGNI）。
- レスポンスは`204 No Content`（既存の`DELETE /api/todos/:id`と同じパターン）。ui側は成功後にクエリキャッシュをinvalidateして再取得する。

## 判断根拠 / 未決事項

- **カード全体をドラッグ入口にする判断**: 6点リーダーを設けず、未完了カードの非インタラクティブ領域をドラッグ入口にする。ボタン・リンク・入力のクリックやフォーカス操作を奪わない。完了・中止カードは一覧末尾に固定し、操作対象から外す。
